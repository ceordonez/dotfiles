		...
		config = function()
			...
			local function save_and_run_async()
				...
				elseif ext == "fcmacro" then
					vim.cmd("Dispatch -compiler=python3 -dir=" .. dir .. " freecadcmd " .. filename)
				elseif ft == "python" then
					...
					local pyexe
					if vim.fn.executable(root_dir .. "/.virtualenvs/bin/python3") == 1 then
						pyexe = root_dir .. "/.virtualenvs/bin/python3"
					else
						pyexe = "python3"
					end
					vim.cmd("Dispatch -compiler=python3 -dir=" .. root_dir .. " " .. pyexe .. " " .. filename)
					vim.cmd("Copen")
				end
			end
